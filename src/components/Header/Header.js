  ...
  return (
    <header className={'header' + (sourceMain ? ' header_about' : '')}>
      <div className='header__nav-link'>
        <button className='header__logo' onClick={handleIconClick} type='button'>
      ...
      {!loggedIn &&
        <div className='header__profile-wrapper'>
          <button className='header__button' onClick={handleRegisterClick} type='button'>Регистрация</button>
          <button className='header__button header__button_fill' onClick={handleLoginClick} type='button'>Войти</button>
        </div>
      }
      {loggedIn &&
        <div className='header__profile-wrapper'>
